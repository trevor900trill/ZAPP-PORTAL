...
import { editcounties } from 'store/reducers/counties';
...
const EditCountyDialog = ({ close, initialDialogValues }) => {
            ...
            {initialDialogValues && (
                <Formik
                    initialValues={{
                        ...
                        isDeleted: false,
                        countyCode: initialDialogValues.countyCode,
                        countyName: initialDialogValues.countyName,
                        regionId: initialDialogValues.regionId //regions.regionsReponse.find((element) => element.id == initialDialogValues.regionId).regionNames
                    }}
                    ...
                    onSubmit={async (values, { setErrors, setStatus, setSubmitting, resetForm }) => {
                        try {
                            setSubmitting(true);
                            var t = await dispatch(editcounties({ id: initialDialogValues.id, ...values }));

                            setStatus({ success: false });
                            setSubmitting(false);

                            if (t.type == 'counties/editcounties/fulfilled') {
                                resetForm();
                                close();
                                await dispatch(fetchcounties());
                            }

                            if (t.type == 'counties/editcounties/rejected') {
