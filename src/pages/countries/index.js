...
import AddCountries from './dialogs/AddCountries';
import EditCountries from './dialogs/EditCountries';
// assets
...
const CountriesPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            var t = await dispatch(fetchcountries());
            if (t.type == 'countries/fetchcountries/rejected') {
                console.log(t);
    ...
    return (
        <Grid container rowSpacing={4.5} columnSpacing={2.75}>
            <Dialog open={open} onClose={handleClose} scroll="body" fullScreen={matchDownMD} TransitionComponent={Transition} keepMounted>
                <DialogContent>
                    <AddCountries close={handleClose} />
                </DialogContent>
            </Dialog>
            ...
            >
                <DialogContent>
                    <EditCountries close={handleEditClose} initialDialogValues={initialDialogValues} />
                </DialogContent>
            </Dialog>
            ...
            <Grid item xs={12} md={12} lg={12}>
                ...
                <MainCard sx={{ mt: 2 }} content={false}>
                    <CountriesTable rows={countries.countriesReponse} modalOpen={handleClickEdit} />
                </MainCard>
            </Grid>
        </Grid>
    );
